  ...
  return (
    <div className="max-w-xl mx-auto job-form-page">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Company*" className="w-full p-2 border rounded" required/>
            ...
            {jobs
              ...
              .map((job) => (
                <li key={job.id} className="border rounded p-3 bg-white shadow">
                  ...
                  <div className="text-sm text-gray-700">Skills: {job.skills.length > 0 ? job.skills.join(", ") : "N/A"}</div>
                  <div className="text-xs text-blue-700 mt-1">Status: {job.status.charAt(0).toUpperCase() + job.status.slice(1)}</div>
